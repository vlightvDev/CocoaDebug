...
@objc public class CocoaDebugSettings: NSObject {
    ...
    override private init() {
        ...
        if let path = Bundle.main.path(forResource: "rsa_public_key", ofType: "pem") {
            do {
                let keyString = try String(contentsOf: URL(fileURLWithPath: path), encoding: .utf8)
                if let keyBase64 = keyString.pemEncodedBase64String(),
                   let keyData = Data(base64Encoded: keyBase64, options: .ignoreUnknownCharacters) {
